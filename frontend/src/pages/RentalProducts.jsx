...
import { Calendar, ShieldCheck, Sparkles, Filter, Search, RotateCcw, Clock } from 'lucide-react';
    ...
    <div className="rental-page">
      ...
      <section className="rental-hero">
        <div className="rental-hero-content container">
          ...
          <p>
            Experience pure luxury without commitment. Rent designer tuxedos, bridal lehengas, silk sarees & gala gowns starting from <strong>₹699 / day</strong>.
          </p>
          <div className="rental-features-bar">
            <div className="rental-feature-tag"><ShieldCheck size={18} /> Refundable Security Deposit</div>
            <div className="rental-feature-tag"><RotateCcw size={18} /> Instant Return & Free Pickup</div>
            <div className="rental-feature-tag"><Clock size={18} /> 3 Units Stock (Emergency Multi-User Renting)</div>
          </div>
        </div>
      </section>
      ...
      <div className="container rental-toolbar-container">
        <div className="rental-category-tabs">
          ...
          <button className={`tab-btn ${category === 'Gala & Evening' ? 'active' : ''}`} onClick={() => setCategory('Gala & Evening')}>Gala & Evening</button>
          <button className={`tab-btn ${category === 'Formal & Luxury Suits' ? 'active' : ''}`} onClick={() => setCategory('Formal & Luxury Suits')}>Luxury Suits</button>
          <button className={`tab-btn ${category === 'Festival & Ethnic' ? 'active' : ''}`} onClick={() => setCategory('Festival & Ethnic')}>Festival & Ethnic</button>
        </div>
        ...
        <form onSubmit={handleSearchSubmit} className="rental-search-box">
          ...
          <input 
            type="text" 
            placeholder="Search rental outfits, brands, designs..." 
            value={search} 
      ...
      <div className="container rental-grid-section">
        {loading ? (
          <div className="rental-loader">
            ...
            <p>Loading Luxora Rental Wardrobe...</p>
          </div>
        ) : rentals.length === 0 ? (
              ...
              <div key={item._id} className="rental-card">
                ...
                <div className="rental-card-info">
                  ...
                  <div className="rental-deposit-row">
                    <span>Refundable Deposit: <strong>₹{item.securityDeposit}</strong></span>
                  </div>
